...
| | |
|---|---|
| Slurm | **38886754** `RUNNING` on **g3129**, 2× H200, account `gpu-h200-harvest` |
...
| Public URL | https://western-doll-rick-rrp.trycloudflare.com |
...
5. Resubmitted **38886754** with `--enable-prompt-tokens-details` so cache-read
   token counts can appear in `usage.prompt_tokens_details.cached_tokens`.
   Cancelled **38852790** only. Confirmed
   `'enable_prompt_tokens_details': True` in `logs/vllm-38886754.log`.
   Public URL: https://western-doll-rick-rrp.trycloudflare.com
...
- Confirm a prefix-hit request reports `usage.prompt_tokens_details.cached_tokens`
  (flag is live on 38886754; not yet client-verified). Not LiteLLM
